...
TEST_F(SelectTest, IntegerLimits) {
    m_con->execute("CREATE TABLE `" + m_table_name + "` (\n"
                   ...
                   ");");
    // min
    m_con->query("INSERT INTO " + m_table_name + " VALUES (0, -128, -32768, -8388608, -2147483648, -9223372036854775808, 0, 0, 0, 0, 0);");
    // max
    m_con->query("INSERT INTO " + m_table_name + " VALUES (1, 127, 32767, 8388607, 2147483647, 9223372036854775807, 255, 65535, 16777215, 4294967295, 18446744073709551615);");
    ...
    EXPECT_EQ(-9223372036854775807 - 1, res->get_signed64(5));      // can't use -9223372036854775808 here (parser limitation), see https://gcc.gnu.org/bugzilla/show_bug.cgi?id=52661
...
TEST_F(SelectTest, RealLimits) {
    ...
    m_con->query("INSERT INTO " + m_table_name + " VALUES (0, -3.402823466e+38, -1.7976931348623157e+308, 1.175494351e-38, 2.2250738585072014e-308);");
    m_con->query("INSERT INTO " + m_table_name + " VALUES (1, -1.175494351e-38, -2.2250738585072014e-308, 3.402823466e+38, 1.7976931348623157e+308);");
    ...
    // 1st row
    EXPECT_FLOAT_EQ(-3.40282e+38, res->get_float(1));       // MariaDB cannot store full single precision -> rounded
    ...
    EXPECT_FLOAT_EQ(3.40282e+38, res->get_float(3));        // MariaDB cannot store full single precision -> rounded
