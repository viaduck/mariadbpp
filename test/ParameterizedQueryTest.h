...
class ParameterizedQueryTest : public SkeletonTest {
    virtual void CreateTestTable() override {
        m_con->execute("CREATE TABLE " + m_table_name +
                       " (id INT AUTO_INCREMENT, preis INT, str VARCHAR(30), nnstr VARCHAR(30) NOT NULL DEFAULT '',"
                       "b BOOL, tim DATETIME(6), tiim TIME(6), d DECIMAL(3,2), dd DOUBLE, nul INT, PRIMARY KEY (id));");
