...
namespace mariadb
{
	namespace concurrency
	{
		...
		//
		extern handle execute(const char* query, bool keep_handle);
		inline void execute(const char* squery) { execute(squery, false); }

		extern handle insert(const char* query, bool keep_handle);
		inline void insert(const char* squery) { insert(squery, false); }

		extern handle query(const char* query, bool keep_handle);
		inline void query(const char* squery) { query(squery, false); }
